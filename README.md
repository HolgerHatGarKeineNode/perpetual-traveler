...
| `pest-plugin-browser` | **installed** (v5.0.1, since 2026-08-12) | Covers the FullCalendar drag-select path in `tests/Browser/CalendarDragSelectTest.php` — the interaction that previously had no test at all. Runs against the **host** Chromium (`/usr/bin/chromium`), not a downloaded binary: run `composer browser:setup` once per machine to wire both Playwright registry entries (`chromium` and `chromium-headless-shell` — headless launches the latter). `tests/Browser` is a `phpunit.xml` suite, so `php artisan test` includes it. The Nostr NIP-07 login stays untestable either way (needs a signer extension). |
